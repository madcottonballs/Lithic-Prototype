...
```		
You can use the 'as' keyword to create an alias.
Ex:
```
import "strarraylib.ltc" as "strarr";
define main() {
	let ptr[5] test;
	strarr.load("hello", 0, test);
	printf(strarr.read(test[0]));
	return(0);
}
```
##	Data:
...
To return multiple objects of different types, either use a tuple or wrap them in a ptr array with the elements being ptr's to your objects.
...
Built-in matrixes (including string arrays) are not supported and will never be supported.
Ex:
>let array x = [3, 5, 7]; /* creates an array 'x' of size 3 preloaded with the numbers 3, 5, and 7 */
Ex:
